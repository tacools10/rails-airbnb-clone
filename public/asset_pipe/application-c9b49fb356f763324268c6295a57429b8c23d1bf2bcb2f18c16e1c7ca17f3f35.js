...
$(document).ready(function() {
  $('#js-show-more-filters-button').click(function(event) {
    event.preventDefault();
    $('#js-show-more-filters-content').toggle();
  });
});
$(document).ready(function() {
  var asset_address = $('#asset_address').get(0);

  if (asset_address) {
    var autocomplete = new google.maps.places.Autocomplete(asset_address, { types: ['geocode'] });
    google.maps.event.addListener(autocomplete, 'place_changed', onPlaceChanged);
    google.maps.event.addDomListener(asset_address, 'keydown', function(e) {
      if (e.keyCode == 13) {
...
function onPlaceChanged() {
  ...
  $('#asset_address').trigger('blur').val(components.address);
  $('#asset_post_code').val(components.zip_code);
  $('#asset_city').val(components.city);
  if (components.country_code) {
    $('#asset_country').val(components.country_code);
  }
}
